const S={cpuHist:Array(20).fill(0),memHist:Array(20).fill(0),plugins:[],sel:null,action:null,loading:false,ws:null,dash:null,theme:localStorage.getItem('theme')||'dark',pollTimer:null};
...
async function api(p,o={}){const r=await fetch(p,{headers:{'Content-Type':'application/json'},...o});const d=await r.json();if(!r.ok)throw new Error(d.error||'HTTP '+r.status);return d}
...
function sparkBars(hist,color){if(!hist||!hist.length)return'';const mx=Math.max(...hist,1);return hist.map(v=>`<div class="spark-bar" style="height:${Math.max(1,v/mx*22)}px;background:${color||'var(--cyan)'}"></div>`).join('')}
...
setInterval(()=>{const c=$('clock');if(c)c.textContent=new Date().toLocaleTimeString();const f=$('footer-time');if(f)f.textContent=new Date().toLocaleTimeString()},1000);
...
function startDash(){
  ...
  if(S.ws)return;
  try{const ws=new WebSocket((location.protocol==='https:'?'wss:':'ws:')+'//'+location.host+'/ws/dashboard');
  ...
  S.ws=ws}catch(e){$('live-dot').style.background='var(--amber)';$('live-label').textContent='轮询'}
}
...
// History
S.cpuHist.push(d.cpu_pct||0);if(S.cpuHist.length>20)S.cpuHist.shift();
const mp=d.mem_total>0?Math.round(d.mem_used/d.mem_total*100):0;
S.memHist.push(mp);if(S.memHist.length>20)S.memHist.shift();
...
const cb=$('cpu-badge');if(cb){cb.textContent=Math.round(cp)+'%';cb.className='mcard-badge '+(cp>90?'mcard-badge-err':cp>70?'mcard-badge-warn':'')}
setT('cpu-model',(d.cpu_model||'-').substring(0,25));
setT('cpu-mhz',Math.round(d.cpu_mhz||0)+' MHz');
setT('cpu-cores',(d.cpu_cores||0)+'核');
...
const cs=$('core-strip');const cores=d.cpu_cores_pct||[];
...
const cpuSpark=$('cpu-spark');if(cpuSpark)cpuSpark.innerHTML=sparkBars(S.cpuHist,'var(--cyan)');

// Memory
$('mem-ring').style.strokeDashoffset=213.6*(1-mp/100);
flash('mem-pct',mp);setT('mem-pct',mp);
const mb=$('mem-badge');if(mb){mb.textContent=mp+'%';mb.className='mcard-badge '+(mp>90?'mcard-badge-err':mp>80?'mcard-badge-warn':'')}
...
setT('mem-avail',fmtB(d.mem_avail*1024));
setT('mem-cached',fmtB(d.mem_cached*1024));
setT('mem-buffers',fmtB(d.mem_buffers*1024));
setT('mem-swap',fmtB(d.swap_used*1024)+'/'+fmtB(d.swap_total*1024));
const memSpark=$('mem-spark');if(memSpark)memSpark.innerHTML=sparkBars(S.memHist,'var(--purple)');
...
const dl=$('disk-list');
if(dl){let h='';(d.disks||[]).forEach(i=>{const p=parseInt(i.pct)||0;const c=p>90?'var(--rose)':p>80?'var(--amber)':'var(--cyan)';h+=`<div class="disk-row"><span class="disk-dev">${esc(i.dev)}</span><span class="disk-mount">${esc(i.mount)}</span><div class="disk-bar"><div class="disk-fill" style="width:${p}%;background:${c}"></div></div><span class="disk-info">${esc(i.used)}/${esc(i.size)} ${esc(i.pct)}</span></div>`});dl.innerHTML=h||'<div class="mcard-empty">无磁盘</div>'}

// Network
const nl=$('net-list');
if(nl){let h='';(d.ifaces||[]).forEach(n=>{h+=`<div class="net-row"><span class="net-name">${esc(n.name)}</span><span class="net-val">↓${fmtB(n.rx_bytes)} ↑${fmtB(n.tx_bytes)}</span></div>`});nl.innerHTML=h||'<div class="mcard-empty">无网络</div>'}
const nb=$('net-badge');if(nb)nb.textContent=(d.ifaces||[]).length+' 接口';
const ni=$('net-ips');if(ni)ni.textContent=d.ips&&d.ips.length?d.ips.join(', '):'—';

// Ports
const pl=$('port-table');
if(pl){let h='<table><tr><th>端口</th><th>协议</th><th>进程</th><th>PID</th></tr>';(d.ports||[]).slice(0,15).forEach(p=>{h+=`<tr><td style="color:var(--cyan);font-weight:600">${p.port}</td><td>${p.proto}</td><td>${esc(p.process||'-')}</td><td>${p.pid||'-'}</td></tr>`});h+='</table>';pl.innerHTML=(d.ports&&d.ports.length)?h:'<div class="mcard-empty">无端口</div>'}
...
(d.apps||[]).forEach(a=>{h+=`<tr><td style="font-weight:500">${esc(a.name.substring(0,30))}</td><td style="color:var(--cyan)">${esc(a.service_name||'-')}</td><td>${a.pid}</td><td><span style="color:var(--primary)">${esc(a.category)}</span></td><td style="color:var(--green)">${a.ports.join(',')||'-'}</td><td style="font-size:11px;color:var(--text3);max-width:100px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${esc(a.exe_path)}">${esc(a.exe_path||'-')}</td><td>${a.threads||'-'}</td></tr>`});
h+='</table>';al.innerHTML=(d.apps&&d.apps.length)?h:'<div class="mcard-empty">无应用</div>'}
...
errs.forEach(e=>{const c=e.severity==='critical'?'var(--rose)':e.severity==='error'?'var(--amber)':'var(--text3)';h+=`<tr><td style="font-weight:500">${esc(e.service)}</td><td style="max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(e.message)}</td><td style="color:${c}">${esc(e.severity)}</td><td style="font-size:11px">${esc(e.timestamp)}</td></tr>`});
h+='</table>';et.innerHTML=h}else{et.innerHTML='<div class="mcard-ok">✓ 无最近错误</div>'}}
const ec=$('err-count');if(ec)ec.textContent=(d.sys_errors||[]).length;

// Anomalies
const an=d.anomalies||[];
const ab=$('alert-badge');
if(ab){if(an.length){ab.textContent=an.length+' 告警';ab.className='mcard-badge mcard-badge-err'}else{ab.textContent='正常';ab.className='mcard-badge mcard-badge-ok'}}
function alertHTML(items,cls){if(!items.length)return'<div class="mcard-ok">✓ 正常</div>';return items.map(a=>`<div class="alert-item ${cls}">⚠ ${esc(a)}</div>`).join('')}
const cpuAn=an.filter(a=>a.includes('CPU')||a.includes('负载'));
const memAn=an.filter(a=>a.includes('内存')||a.includes('Swap'));
const diskAn=an.filter(a=>a.includes('磁盘')||a.includes('FD')||a.includes('分区'));
const procAn=an.filter(a=>a.includes('进程')||a.includes('僵尸'));
const acpu=$('alert-cpu');if(acpu)acpu.innerHTML=alertHTML(cpuAn,'cpu');
const amem=$('alert-mem');if(amem)amem.innerHTML=alertHTML(memAn,'mem');
const adisk=$('alert-disk');if(adisk)adisk.innerHTML=alertHTML(diskAn,'disk');
const aproc=$('alert-proc');if(aproc)aproc.innerHTML=alertHTML(procAn,'proc');
// Footer
...
function renderPL(){const q=($('p-search')?.value||'').toLowerCase();$('p-list').innerHTML=S.plugins.filter(p=>!q||p.name.toLowerCase().includes(q)||p.category.toLowerCase().includes(q)).map(p=>`<div class="p-item${S.sel===p.name?' active':''}" onclick="selP('${esc(p.name)}')"><div class="p-icon ${piCls(p.category)}">${piCh(p.category)}</div><div class="p-info"><div class="p-name">${esc(p.name)}</div><div class="p-meta">${esc(p.category)} · v${esc(p.version)}</div></div></div>`).join('')||'<div class="mcard-empty">无插件</div>'}
...
$('act-grid').innerHTML=p.actions.length?p.actions.map(a=>`<button class="act-btn${S.action===a.name?' active':''}" onclick="selA('${esc(a.name)}')">${esc(a.name)}</button>`).join(''):'<div class="mcard-empty">无操作</div>';
...
D.containers.forEach(c=>{const r=c.status.includes('Up');h+=`<tr><td style="font-family:monospace;font-size:11px">${esc(c.id.substring(0,12))}</td><td style="font-weight:500">${esc(c.name)}</td><td>${esc(c.image)}</td><td class="${r?'text-ok':'text-err'}">${r?'●':'○'} ${esc(c.status)}</td><td>${esc(c.cpu)}</td><td>${esc(c.mem)}</td><td class="docker-acts"><button class="btn btn-sm btn-ghost" onclick="D.act('${c.id}','${r?'stop':'start'}')">${r?'停':'启'}</button><button class="btn btn-sm btn-ghost" onclick="D.act('${c.id}','restart')">重启</button><button class="btn btn-sm btn-ghost" onclick="D.logs('${c.id}')">日志</button><button class="btn btn-sm btn-ghost" onclick="D.inspect('${c.id}')">详情</button></td></tr>`});
h+='</table>';$('d-containers').innerHTML=D.containers.length?h:'<div class="mcard-empty">无容器</div>'}catch(e){$('d-containers').innerHTML='<div class="mcard-empty" style="color:var(--rose)">Docker 不可用</div>'}},
async loadImages(){try{const d=await api('/api/docker/images');let h='<table><tr><th>仓库</th><th>标签</th><th>ID</th><th>大小</th></tr>';
(d.images||[]).forEach(i=>{h+=`<tr><td>${esc(i.repo)}</td><td>${esc(i.tag)}</td><td style="font-family:monospace;font-size:11px">${esc(i.id.substring(0,12))}</td><td>${esc(i.size)}</td></tr>`});h+='</table>';$('d-images').innerHTML=h||'<div class="mcard-empty">无镜像</div>'}catch(e){}},
async loadCompose(){try{const d=await api('/api/docker/compose');const ps=d.projects||[];
if(!ps.length){$('d-compose').innerHTML='<div class="mcard-empty">无 Compose 项目</div>';return}
let h='';ps.forEach(p=>{h+=`<div style="margin-bottom:8px;border:1px solid var(--border);border-radius:6px;overflow:hidden"><div style="padding:8px 12px;background:var(--bg3);display:flex;align-items:center;gap:8px"><span style="font-weight:600;font-size:13px">${esc(p.name)}</span><span class="mcard-badge">${esc(p.status)}</span><span style="flex:1"></span><button class="btn btn-sm btn-ghost" onclick="D.composeAct('${esc(p.name)}','up')">启动</button><button class="btn btn-sm btn-ghost" onclick="D.composeAct('${esc(p.name)}','down')">停止</button><button class="btn btn-sm btn-ghost" onclick="D.composeAct('${esc(p.name)}','restart')">重启</button><button class="btn btn-sm btn-ghost" onclick="D.composeLogs('${esc(p.name)}')">日志</button></div><table><tr><th>服务</th><th>镜像</th><th>状态</th><th>端口</th></tr>`;
p.services.forEach(s=>{const r=s.status.includes('Up');h+=`<tr><td style="font-weight:500">${esc(s.name)}</td><td>${esc(s.image)}</td><td class="${r?'text-ok':'text-err'}">${r?'●':'○'} ${esc(s.status)}</td><td>${esc(s.ports||'-')}</td></tr>`});h+='</table></div>'});$('d-compose').innerHTML=h}catch(e){$('d-compose').innerHTML='<div class="mcard-empty" style="color:var(--rose)">加载失败</div>'}},
...
renderConns(){if(!this.config||!this.sel){$('mw-conn').innerHTML='<div class="mcard-empty">暂无</div>';return}
const c=this.config[this.sel]||[];if(!c.length){$('mw-conn').innerHTML='<div class="mcard-empty">暂无连接</div>';return}
let h='<table><tr><th>名称</th><th>地址</th><th>操作</th></tr>';c.forEach(x=>{const a=x.host?x.host+':'+(x.port||''):x.brokers||x.config_path||'-';h+=`<tr><td style="font-weight:500">${esc(x.name)}</td><td style="font-family:monospace;font-size:11px">${esc(a)}</td><td><button class="btn btn-sm btn-danger" onclick="MW.remove('${esc(x.name)}')">删除</button></td></tr>`});h+='</table>';$('mw-conn').innerHTML=h},
...
async discover(){try{const d=await api('/api/middleware/discover');const s=d.services||[];if(!s.length){$('mw-content').innerHTML='<div class="mcard-empty">未发现</div>';return}
